...
export function Details() {
  const [optionSelected, setOptionSelected] = useState<"guesses" | "rank">(
    "guesses"
  );
  ...
  async function fetchPollDetails() {
    try {
      ...
      console.log(error);
      toast.show({
        title: "We were not able to load your poll's details, try again!",
        placement: "top",
        bgColor: "red.500",
      });
    } finally {
      ...
      {pollDetails._count?.participants > 0 ? (
        <VStack px={5} flex={1}>
          ...
          <HStack bgColor="gray.800" p={1} rounded="sm" mb={5}>
            ...
            <Option
              title="Group Rank"
              isSelected={optionSelected === "rank"}
              onPress={() => setOptionSelected("rank")}
            />
          </HStack>
